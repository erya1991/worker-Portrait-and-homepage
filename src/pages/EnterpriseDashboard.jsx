...
function MetricCard({ icon: Icon, tone, title, value, suffix, desc }) {
  return <div className="enterprise-metric-card min-w-0 rounded border border-border-gray bg-white px-3 py-2.5 shadow-sm"><div className="flex items-center justify-between gap-2"><span className="truncate text-xs text-text-secondary">{title}</span><span className={`rounded p-1 ${metricTones[tone]}`}><Icon className="h-3.5 w-3.5" /></span></div><div className="mt-2 flex min-w-0 items-baseline gap-1"><span className="truncate text-[21px] font-bold leading-none tracking-tight">{value}</span>{suffix && <span className="shrink-0 text-[11px] text-text-secondary">{suffix}</span>}</div><div className="mt-1.5 truncate text-[11px] text-text-secondary">{desc}</div></div>
}
...
function WorkerPoolPanel({ workerPool }) {
  return <div className="enterprise-panel flex h-full min-h-0 flex-col rounded border border-border-gray bg-white p-3.5 shadow-sm"><PanelTitle icon={UsersRound} title="企业工人库" action={<span className="text-xs font-normal text-text-secondary">标签与等级分布</span>} /><div className="grid min-h-0 flex-1 grid-cols-[minmax(205px,0.95fr)_minmax(220px,1.05fr)] gap-3"><div className="flex min-h-0 flex-col"><div className="mb-2 text-xs font-medium">A/B/C/D 等级分布</div><div className="min-h-0 flex-1"><GradeChart grades={workerPool.grades} /></div></div><div className="flex min-h-0 flex-col"><div className="mb-2 text-xs font-medium">标签人数</div><div className="grid min-h-0 flex-1 grid-cols-2 grid-rows-2 gap-2">{workerPool.tags.slice(0, 4).map((tag) => <div key={tag.tagId} className={`enterprise-tag-stat flex min-h-0 flex-col justify-center rounded border px-2.5 py-2 ${tag.property === '警示' ? 'border-[#FFCCC7] bg-[#FFF1F0]' : 'border-[#D6E4FF] bg-[#F6FAFF]'}`}><div className="truncate text-[11px] text-text-secondary" title={tag.name}>{tag.name}</div><div className={`mt-1 text-lg font-bold ${tag.property === '警示' ? 'text-danger-red' : 'text-primary'}`}>{formatNumber(tag.value)}<span className="ml-0.5 text-[10px] font-normal text-text-secondary">人</span></div></div>)}</div></div></div></div>
}
...
export default function EnterpriseDashboard({ onOpenProject = () => {} }) {
  ...
  const metrics = [
    ...
    { icon: UsersRound, tone: 'blue', title: '工人库人数', value: formatNumber(summary.workerPool.totalWorkerCount), suffix: '人', desc: summary.workerPool.scoreAvailable ? `平均分 ${summary.workerPool.averageScore}` : '暂无评价' },
