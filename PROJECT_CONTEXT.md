...
2. **数据采集中心**：十个平级业务页签，包括人员信息、用工信息、考勤信息、班组信息、企业信息、体检记录、奖励记录、处罚记录、证书记录和劳资纠纷；AI 智能采集作为五类业务记录页签内的操作；AI 结果直接进入对应台账并通过“待确认”状态和快捷筛选处理。
...
数据采集中心的批量 AI 采集当前使用页面内存 Mock：从当前业务记录页签选择多个附件，模拟异步识别状态，并按当前台账类型填充对应字段；成功或部分识别结果直接进入当前台账并标记“待确认”，手动新增记录标记“无需确认”，管理人员通过勾选“仅看待确认”筛选和左右对照确认弹窗编辑后确认，状态变为“已确认”。不再存在独立待确认列表、驳回状态或驳回操作。真实文件存储、OCR 服务、识别置信度、人员/项目匹配和去重接口仍待后端联调。
