...
2. **数据采集中心**：实名制数据、AI 智能采集、待确认数据、人工数据维护。
...
- 实名制数据：人员信息、用工信息、考勤信息、班组信息、企业信息；
- 人工数据维护：体检记录、奖励记录、处罚记录、证书记录、劳资纠纷。
...
当前已验证 `npm.cmd run build` 成功。`lint` 中存在 `DataCenter.jsx` 的既有未使用导入警告，不影响当前运行入口。
